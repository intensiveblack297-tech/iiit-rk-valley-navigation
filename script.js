...
// Prototype campus model:
// AB-1/AB-2 contain smaller departments such as Maths, Physics, Chemistry, English and Telugu.
// CSE/ECE/EEE/Civil/MME/ME are modeled as separate department blocks.
// Hostels contain residential/common facilities only; no labs.
const blocks = [
  {name:'AB-1',icon:'🏢',query:'Academic Block 1, IIIT RGUKT RK Valley',type:'academic',smallDepartments:['Mathematics','Physics','Chemistry','English','Telugu','Management','Library Science','Physical Education']},
  {name:'AB-2',icon:'🏢',query:'Academic Block 2, IIIT RGUKT RK Valley',type:'academic',smallDepartments:['Mathematics','Physics','Chemistry','English','Telugu','Management','Library Science','Physical Education']},
  {name:'CSE Block',icon:'💻',query:'CSE Department, IIIT RGUKT RK Valley',type:'department',department:'Computer Science and Engineering'},
  {name:'ECE Block',icon:'📡',query:'ECE Department, IIIT RGUKT RK Valley',type:'department',department:'Electronics and Communication Engineering'},
  {name:'EEE Block',icon:'⚡',query:'EEE Department, IIIT RGUKT RK Valley',type:'department',department:'Electrical and Electronics Engineering'},
  {name:'Civil Block',icon:'🏗️',query:'Civil Engineering Department, IIIT RGUKT RK Valley',type:'department',department:'Civil Engineering'},
  {name:'MME Block',icon:'🔩',query:'MME Department, IIIT RGUKT RK Valley',type:'department',department:'Materials Science and Metallurgical Engineering'},
  {name:'ME Block',icon:'⚙️',query:'Mechanical Engineering Department, IIIT RGUKT RK Valley',type:'department',department:'Mechanical Engineering'},
  {name:'CHE Block',icon:'🧪',query:'Chemical Engineering Department, IIIT RGUKT RK Valley',type:'department',department:'Chemical Engineering'},
  {name:'IT Block',icon:'💾',query:'IT Department, IIIT RGUKT RK Valley',type:'department',department:'Information Technology'},
...
const academicFloors = [
  ['101','102','103','104','105 — Mathematics','106 — Physics','107 — Chemistry','108 — English','109 — Telugu','110 — Academic Office'],
  ['201','202','203','204','205 — Mathematics','206 — Physics','207 — Chemistry','208 — English','209 — Telugu','210 — Faculty / Staff Room'],
  ['301','302','303','304','305 — Management','306 — Library Science','307 — Physical Education','308 — Faculty Room','309 — Seminar Room','310 — Common Area']
];
const departmentFloors = [
  ['101 — Department Office','102 — Faculty Room','103 — Classroom','104 — Classroom','105 — Classroom','106 — Classroom','107 — Classroom','108 — Student Room','109 — Meeting Room','110 — Common Area'],
  ['201 — Classroom','202 — Classroom','203 — Classroom','204 — Classroom','205 — Teaching Lab','206 — Teaching Lab','207 — Project Lab','208 — Faculty Room','209 — Research Room','210 — Seminar Room'],
  ['301 — Classroom','302 — Classroom','303 — Classroom','304 — Classroom','305 — Seminar Hall','306 — Project Lab','307 — Research Lab','308 — Faculty Room','309 — Meeting Room','310 — Student Activity Room']
];
const hostelFloors = [
...
function floorsFor(b){if(blocks[b].type==='hostel')return hostelFloors;if(blocks[b].type==='sports')return sacFloors;if(blocks[b].type==='department')return departmentFloors;return academicFloors;}
...
function mapsDirectionsUrl(toBlock){const origin=encodeURIComponent(`${campusCenter}, IIIT RGUKT RK Valley, Idupulapaya, Andhra Pradesh`);const destination=encodeURIComponent(`${blocks[toBlock].query}, Idupulapaya, Andhra Pradesh`);return `https://www.google.com/maps/dir/?api=1&origin=${origin}&destination=${destination}&travelmode=walking`;}
...
function setMapsButtons(toBlock){googleMaps.hidden=false;googleMaps.textContent='🌍 Open Outdoor Route in Google Maps';googleMaps.onclick=()=>window.open(mapsDirectionsUrl(toBlock),'_blank','noopener,noreferrer');document.getElementById('mapsSearchFallback')?.remove();const searchButton=document.createElement('button');searchButton.id='mapsSearchFallback';searchButton.className='secondary-button';searchButton.textContent='📍 Search Destination in Google Maps';searchButton.onclick=()=>window.open(mapsSearchUrl(blocks[toBlock].query),'_blank','noopener,noreferrer');googleMaps.parentElement.appendChild(searchButton);}
document.getElementById('navigate').addEventListener('click',()=>{const cb=+currentBuilding.value,cf=+currentFloor.value,cr=+currentRoom.value,db=+destinationBuilding.value,df=+destinationFloor.value,dr=+destinationRoom.value;routeResult.hidden=false;steps.innerHTML='';floorPreview.innerHTML='';googleMaps.hidden=true;document.getElementById('mapsSearchFallback')?.remove();message.textContent='';if(cb===db&&cf===df&&cr===dr){routeSummary.textContent=`You are already at ${placeName(db,df,dr)}.`;addStep('No route is needed — your current location and destination are the same.');renderFloorMap(db,df,cr,dr,false);return;}routeSummary.textContent=`${placeName(cb,cf,cr)} → ${placeName(db,df,dr)}`;addStep(`Start at ${placeName(cb,cf,cr)}.`);if(cb===db&&cf===df){addStep('Follow the calculated blue A* route through the corridor.');addStep(`Arrive at ${roomsFor(db,df)[dr]} 🎯.`);renderFloorMap(db,df,cr,dr,true);return;}if(cb===db){addStep('Follow the indoor route to the nearest stairs/elevator.');addStep(`Take the stairs/elevator from ${floorLabel(cb,cf)} to ${floorLabel(db,df)}.`);addStep(`Follow the corridor on ${floorLabel(db,df)} to ${roomsFor(db,df)[dr]}.`);renderFloorMap(db,df,-1,dr,false);return;}addStep(`Follow the indoor route to the ${blocks[cb].name} entrance/exit.`);addStep(`For the outdoor section, open Google Maps and navigate from the campus area to ${blocks[db].name}.`);addStep(`At ${blocks[db].name}, go to ${floorLabel(db,df)} and follow the indoor route to ${roomsFor(db,df)[dr]}.`);setMapsButtons(db);renderFloorMap(db,df,-1,dr,false);});