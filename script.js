...
const blocks = [
  ...
  {name:'CE Block',icon:'🏗️',query:'Civil Engineering Department, IIIT RGUKT RK Valley',type:'department',department:'Civil Engineering'},
  ...
  {name:'Science Block',icon:'🔬',query:'Science Department, IIIT RGUKT RK Valley',type:'department',department:'Mathematics, Physics and Chemistry'},
...
const academicFloors = [
  ['101','102','103','104','105 — Department Office','106 — Faculty Room','107 — Classroom','108 — Classroom','109 — Seminar Room','110 — Common Area'],
  ['201','202','203','204','205 — Teaching Lab','206 — Teaching Lab','207 — Project Lab','208 — Faculty Room','209 — Classroom','210 — Department Office'],
  ['301','302','303','304','305 — Seminar Hall','306 — Project Lab','307 — Faculty Room','308 — Classroom','309 — Meeting Room','310 — Common Area']
];
...
function mapsDirectionsUrl(fromBlock,toBlock){const origin=encodeURIComponent(`${campusCenter} (${blocks[fromBlock].name}), IIIT RGUKT RK Valley`);const destination=encodeURIComponent(blocks[toBlock].query);return `https://www.google.com/maps/dir/?api=1&origin=${origin}&destination=${destination}&travelmode=walking`;}
...
blocks.forEach(b=>{const d=document.createElement('div');d.className='place';const description=b.type==='hostel'?'3 floors • residential rooms • warden/common/study spaces • no labs':b.type==='sports'?'Student Activity Centre • gym • tennis • basketball • badminton • indoor games':b.type==='department'?`Dedicated department block • ${b.department} • classrooms • labs • faculty/research spaces`:'3 floors • departments • classrooms • labs • faculty/academic facilities';d.innerHTML=`<strong>${b.icon} ${b.name}</strong><span>${description}</span>`;locations.appendChild(d);});
